<!--
  ✨ zl1nq · GitHub 个人主页 README

  使用说明（这段注释不会显示在主页上）：
  1️⃣ 把本仓库推送到 GitHub：仓库名必须是 zl1nq（与用户名一致），并设为 Public
  2️⃣ 默认分支（一般是 main）上的 README.md 会自动渲染到你的个人主页顶部
  3️⃣ 文中标有 ✏️ 的位置，建议替换成你自己的真实信息
  4️⃣ 「贪吃蛇」首次需要手动触发：仓库 Actions 页 → Generate Snake → Run workflow
-->

<div align="center">
  <img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=0:8A2BE2,50:E879F9,100:00E5FF&height=230&section=header&text=Hi%2C%20I%27m%20zl1nq&fontSize=60&fontColor=ffffff&fontAlignY=36&desc=Build%20%C2%B7%20Explore%20%C2%B7%20Ship&descSize=18&descAlignY=56&animation=fadeIn" alt="banner"/>
</div>

<div align="center">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=700&size=26&pause=1000&color=E879F9&center=true&vCenter=true&width=720&height=80&lines=Hello%2C+world!;Exploring+AI+%2B+Code;Talk+is+cheap.+Show+me+the+code.;Keep+building%2C+keep+shipping." alt="typing"/>
</div>

<div align="center">
  <img src="https://komarev.com/ghpvc/?username=zl1nq&style=for-the-badge&color=blueviolet&label=%E8%AE%BF%E5%AE%A2" alt="visitors"/>
  <img src="https://img.shields.io/github/followers/zl1nq?style=for-the-badge&color=blueviolet&label=FOLLOWERS" alt="followers"/>
  <img src="https://img.shields.io/github/stars/zl1nq?affiliations=OWNER&style=for-the-badge&color=fuchsia&label=STARS" alt="stars"/>
</div>

## 🧭 关于我

<table>
<tr>
<td valign="top" width="50%">

### 🚀 现在的样子

- 🔭 正在做 **AI + 代码** 相关的探索和个人项目
- 🌱 正在学 **更优雅的工程化与前端体验**
- 👯 希望和有趣的灵魂一起搞点东西
- 💬 欢迎聊聊 **AI 辅助编程 / 效率工具 / 一切好玩的点子**
- ⚡ 信条：**把想法变成现实，是程序员最浪漫的事**

</td>
<td valign="top" width="50%">

### 🎯 近期目标

- ✍️ 把草稿箱里的 idea 一个个搬到 GitHub 上落地
- 🧪 玩转 AI 编程工作流，让重复劳动统统自动化
- 📚 保持输入：源码、文档，还有一点点哲学
- 🌍 让作品被更多人看到 —— 所以才有了你现在看到的这个页面

</td>
</tr>
</table>

<details>
<summary><b>🥚 点我查看一个小彩蛋</b></summary>
<br/>

```python
while alive:
    eat()
    sleep()
    code()
    repeat()  # 人生就是一个大循环
```

</details>

## 🛠️ 技术栈

<!-- ✏️ 图标可自由增删，完整清单见：https://github.com/tandpfun/skill-icons -->

<div align="center">
  <img src="https://skillicons.dev/icons?i=python,ts,js,react,vue,nodejs,html,css&theme=dark" height="46" alt="languages"/>
  <br/>
  <img src="https://skillicons.dev/icons?i=git,githubactions,docker,linux,vscode,md,vercel,figma&theme=dark" height="46" alt="tools"/>
</div>

## 📊 GitHub 战绩

<div align="center">
  <img height="170" src="https://github-readme-stats.vercel.app/api?username=zl1nq&show_icons=true&theme=radical&hide_border=true&bg_color=0D1117&include_all_commits=true&count_private=true" alt="github stats"/>
  <img height="170" src="https://github-readme-stats.vercel.app/api/top-langs/?username=zl1nq&layout=compact&theme=radical&hide_border=true&bg_color=0D1117&langs_count=8" alt="top languages"/>
  <br/>
  <img height="170" src="https://streak-stats.demolab.com?user=zl1nq&theme=radical&hide_border=true&background=0D1117" alt="streak stats"/>
</div>

<div align="center">
  <img width="100%" src="https://github-readme-activity-graph.vercel.app/graph?username=zl1nq&bg_color=0D1117&color=E879F9&line=8A2BE2&point=00E5FF&area=true&area_color=8A2BE2&hide_border=true" alt="activity graph"/>
</div>

## 🏆 成就墙

<div align="center">
  <img width="100%" src="https://github-profile-trophy.vercel.app/?username=zl1nq&theme=radical&no-frame=true&row=1&column=7&margin-w=6" alt="trophies"/>
</div>

## 🐍 贡献贪吃蛇

<!-- ✏️ 首次推送后，到 Actions 页手动运行一次 Generate Snake，之后每天自动更新 -->

<div align="center">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/zl1nq/zl1nq/output/github-contribution-grid-snake-dark.svg"/>
    <img width="100%" src="https://raw.githubusercontent.com/zl1nq/zl1nq/output/github-contribution-grid-snake.svg" alt="contribution snake"/>
  </picture>
</div>

## 📮 找到我

<div align="center">

[![GitHub](https://img.shields.io/badge/GitHub-zl1nq-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/zl1nq)

<!-- ✏️ 想加邮箱/微信/QQ/知乎/掘金？仿照下面这行，去掉注释并填上你的信息即可：
[![Email](https://img.shields.io/badge/Email-your%40email.com-EA4335?style=for-the-badge&logo=gmail&logoColor=white)](mailto:your@email.com)
-->

</div>

> 💡 *如果我的项目对你有帮助，欢迎点一个 ⭐ —— 那是我持续输出的燃料。*

<div align="center">
  <img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=0:00E5FF,50:E879F9,100:8A2BE2&height=140&section=footer&text=THANKS%20FOR%20VISITING&fontSize=26&fontColor=ffffff&fontAlignY=68&animation=fadeIn" alt="footer"/>
</div>
